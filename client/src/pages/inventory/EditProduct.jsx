...
import { Package, Save, ArrowLeft, Loader2, AlertTriangle } from "lucide-react";
...
const EditProduct = () => {
  ...
  const [formData, setFormData] = useState({
    ...
    quantity: "", unit: "", supplier: "", lowStockThreshold: "",
  });

  useEffect(() => {
    const fetchProduct = async () => {
      try {
        const p = await getProductById(id); // ✅ returns product directly now
  ...
  const handleSubmit = async (e) => {
    ...
    if (!formData.name || !formData.category || !formData.costPrice ||
        !formData.sellingPrice || !formData.quantity || !formData.unit) {
          ...
          <form onSubmit={handleSubmit}>
            <div className="glass-card rounded-2xl p-6 space-y-6">
              ...
              <div className="grid grid-cols-2 gap-4">
                <Input type="number" placeholder="Cost Price" value={formData.costPrice}
                  onChange={(e) => setFormData({ ...formData, costPrice: e.target.value })} />
                <Input type="number" placeholder="Selling Price" value={formData.sellingPrice}
              ...
              <Input placeholder="Supplier" value={formData.supplier}
